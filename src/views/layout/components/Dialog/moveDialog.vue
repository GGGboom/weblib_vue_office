<template>
  <el-dialog v-el-drag-dialog title="移动到" :visible.sync="visible" width="450px" class="copyDialog" @open="openCallback">

    <el-tabs v-model="activeName" v-if="isInit">
      <el-tab-pane label="个人资源库" name="first">
        <folder-tree :hidden-back-btn=false :id="personalGroupId" :CopyorMove=1></folder-tree>
      </el-tab-pane>
...
<script>
  import { createDir, moveResource } from '@/api/resources'
  ...
  export default {
    ...
    created(){
      this.$root.$on('open-move-dialog', (data) => {
    ...
    methods: {
      handleSubmit(){
        ...
        moveResource({
          ...
          this.$message({ showClose: true, message: '移动成功！', type: 'success', duration: 1000 })
          this.visible = false
          this.$root.$emit('move-success-callback')
        })
      },
