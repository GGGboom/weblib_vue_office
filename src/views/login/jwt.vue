<template>
  <div class="login-container" :style="login_bg">
    <el-form class="login-form" autoComplete="on" :model="loginForm" :rules="loginRules"
      ...
      <!--</el-select>-->
      <!--</div>-->
      <el-button class="loginBtn" type="primary" style="width:100%;margin-bottom:30px;"
                 :loading="loading"
...
<script>
  ...
  import { getUserInfo, newLogin, getSystemVersion, jwtToken, checkLogin } from '@/api/login'
          ...
          if (valid) {
            ...
            if (this.version) {
              ...
              newLogin({
                ...
                res = res.data
                if (res.code === '300') {
                  this.$message({ showClose: true, message: res.message, type: 'warning', duration: 1000 })
                }
                if (res.code === '200') {
                  jwtToken({
                    username: this.loginForm.username,
                    ...
                    subRes = subRes.data
                    console.log(subRes.access_token)
                    if (subRes.token_type === 'bearer') {
                      setToken(subRes.access_token) //设置登录token
//                      this.$store.dispatch('setToken', subRes.access_token)
...
//                        password: this.loginForm.password
//                      }, subRes.access_token), then(res => {
//                        console.log(res)
//                      })
                    }
                  }).catch(e => {
                    this.$message({ showClose: true, message: '登录失败', type: 'warning', duration: 1000 })
                  })
                  this.$message({ showClose: true, message: res.message, type: 'warning', duration: 1000 })
                }
//                if (res.message === '登陆成功') {
...
//                }
    ...
    created() {
      ...
      getSystemVersion().then(res => {
        res = res.data
        if (parseInt(res.systemVersion.split('.')[0]) >= 5) {
          this.version = true
        } else {
          this.$message({ showClose: true, message: '该版本不支持微服务登录！', type: 'warning', duration: 1000 })
          this.$router.push({ path: '/' })
          this.version = false
        }
        this.isDiabledButton = false
      }).catch(error => {
        this.isDiabledButton = false
      })
    },
